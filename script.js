...
function bibleEntries(){
  const b=getBrandBible(); let raw=[];
  if(Array.isArray(b.entries)) raw=b.entries;
  else if(b.entries&&typeof b.entries==='object') raw=Object.values(b.entries);
  else { const legacy=getBrandResources()?.notes; if(legacy) raw=[{section:'Things to Check',text:legacy}]; }
  const out=[];
  raw.filter(Boolean).forEach(x=>{
    const section=normalizeBibleSection(x.section);
    if(x.category && Array.isArray(x.bullets)){
      const bullets=x.bullets.map(cleanBibleBullet).filter(Boolean);
      if(bullets.length) out.push({section,category:String(x.category).trim()||'General',bullets,text:bullets.join('\n')});
      return;
    }
...
function openBrandBible(){
  ...
  renderBrandBibleWorkspace(); document.querySelector('#brandBibleDialog').showModal();
}
function renderBrandBibleWorkspace(){
  ...
  DEFAULT_BIBLE_SECTIONS.forEach(section=>{
    ...
    if(sectionEntries.length) sectionEntries.forEach(x=>addBibleCategoryRow(section,x.category,x.bullets,wrap));
...
function addBibleCategoryRow(section,category='',bullets=[],target=null){
...
async function saveBrandBible(e){
  ...
  for(const r of rows){ const section=normalizeBibleSection(r.dataset.section), category=r.querySelector('.bible-category-input')?.value.trim()||'', bullets=String(r.querySelector('.bible-bullets-input')?.value||'').split(/\r?\n/).map(cleanBibleBullet).filter(Boolean); if(!category&&!bullets.length)continue; if(!category)return alert('Please add a category / heading for each Brand Bible group.'); if(!bullets.length)return alert(`Please add at least one bullet under ${category}.`); entries.push({section,category,bullets}); }
  const ok=await write(`brandBibles/${brandKey(currentBrand)}`,{version:2,categories:[...DEFAULT_BIBLE_SECTIONS],entries,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Bible saved'); if(ok)document.querySelector('#brandBibleDialog').close();
}
...
function brandBibleSectionHtml(section,entries){
  ...
  return `<section class="content-rule-section"><strong>${escapeHtml(section)}</strong>${groups.map(g=>`<div class="content-rule-category"><b>${escapeHtml(g.category)}</b><ul>${g.bullets.map(line=>`<li>${escapeHtml(line)}</li>`).join('')}</ul></div>`).join('')}</section>`;
}
