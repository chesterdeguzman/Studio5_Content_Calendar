...
const els = {
  ...
  brandBibleBtn: document.querySelector("#brandBibleBtn"), brandBibleDialog: document.querySelector("#brandBibleDialog"), brandBibleForm: document.querySelector("#brandBibleForm"), brandBibleSections: document.querySelector("#brandBibleSections"), addBrandBibleSectionBtn: document.querySelector("#addBrandBibleSectionBtn"), closeBrandBibleBtn: document.querySelector("#closeBrandBibleBtn"), cancelBrandBibleBtn: document.querySelector("#cancelBrandBibleBtn"),
  addSopLinkBtn: document.querySelector("#addSopLinkBtn"), sopPdfInput: document.querySelector("#sopPdfInput"), sopResourceList: document.querySelector("#sopResourceList"), sopLinkDialog: document.querySelector("#sopLinkDialog"), sopLinkForm: document.querySelector("#sopLinkForm"), closeSopLinkBtn: document.querySelector("#closeSopLinkBtn"),
  qcChecklist: document.querySelector("#qcChecklist"), addQcItemBtn: document.querySelector("#addQcItemBtn")
};
...
function openEditDialog(id) {
  ...
  const fields = {
    ...
    contentGoal: item.goal || "", contentProductFocus: item.productFocus || "", contentHook: item.hook || "", contentWhyCare: item.whyCare || "", contentDirection: item.direction || "", contentCta: item.cta || "", contentAssets: item.assets || "", contentReferences: item.references || "", contentAvoid: item.avoid || "",
...
async function saveContentFromForm(event) {
  ...
  const item = {
    ...
    goal: document.querySelector("#contentGoal").value.trim(), productFocus: document.querySelector("#contentProductFocus").value.trim(), hook: document.querySelector("#contentHook").value.trim(), whyCare: document.querySelector("#contentWhyCare").value.trim(), direction: document.querySelector("#contentDirection").value.trim(), cta: document.querySelector("#contentCta").value.trim(), assets: document.querySelector("#contentAssets").value.trim(), references: document.querySelector("#contentReferences").value.trim(), avoid: document.querySelector("#contentAvoid").value.trim(),
    ...
    qc: readQcEditor(), creatorCheck: document.querySelector("#creatorCheck").checked, internalReview: document.querySelector("#internalReview").checked, nicoleApproval: document.querySelector("#nicoleApproval").checked, readyForClient: document.querySelector("#readyForClient").checked, revisionReason: document.querySelector("#revisionReason").value, revisionOther: document.querySelector("#revisionOther").value.trim(),
    updatedAt: Date.now(),
    updatedBy: auth.currentUser.uid
  };
...
function bindEvents() {
  ...
  document.querySelectorAll(".nav-item").forEach((button) => button.addEventListener("click", () => {
    ...
    document.querySelector("#pageTitle").textContent = { calendar: "Monthly Content Calendar", special: "Special Days Calendar", overview: "Brand Overview", links: "Content Links", sop: "Brand SOP Library", usecase: "Use Case Diagram", settings: "Settings" }[button.dataset.view];
  }));
  ...
  els.brandBibleBtn?.addEventListener("click", openBrandBible); els.brandBibleForm?.addEventListener("submit", saveBrandBible); els.addBrandBibleSectionBtn?.addEventListener("click", () => addBrandBibleSection()); els.closeBrandBibleBtn?.addEventListener("click", () => els.brandBibleDialog.close()); els.cancelBrandBibleBtn?.addEventListener("click", () => els.brandBibleDialog.close());
  els.addSopLinkBtn?.addEventListener("click", () => els.sopLinkDialog.showModal()); els.closeSopLinkBtn?.addEventListener("click", () => els.sopLinkDialog.close()); els.sopLinkForm?.addEventListener("submit", saveSopLink); els.sopPdfInput?.addEventListener("change", saveSopPdfs);
  els.addQcItemBtn?.addEventListener("click", () => addQcRow("New QC item", false)); document.querySelector("#contentType")?.addEventListener("change", () => ensureReelQc()); ["creatorCheck","internalReview","nicoleApproval"].forEach(id => document.querySelector(`#${id}`)?.addEventListener("change", syncQualityGate));
}
...
const DEFAULT_INTERNAL_QC = ["Client brief followed exactly","Brand Bible followed","UK English checked","Correct fonts, colours and brand style","Facts/contact details verified","Latest relevant imagery used","One clear content focus","Hook is strong and specific","Caption matches creative","CTA checked","No prohibited content/assets","Final spelling/proofing completed"];
function getBrandBible(){ return remote.brandBibles?.[brandKey(currentBrand)] || {}; }
function openBrandBible(){ els.brandBibleSections.innerHTML=""; const bible=getBrandBible(); const sections=Object.values(bible); if(sections.length) sections.forEach(x=>addBrandBibleSection(x)); else ["Brand Overview","Non-Negotiables","Visual Guidelines","Copy & Tone","Content Priorities","Do / Don't","Important Business Information","Additional Notes"].forEach(title=>addBrandBibleSection({title,text:title==="Copy & Tone"?"UK English throughout.":"",mandatoryQc:title==="Non-Negotiables"})); els.brandBibleDialog.showModal(); }
function addBrandBibleSection(section={title:"New Section",text:"",mandatoryQc:false}){ const row=document.createElement("div"); row.className="brand-resource-file"; row.style.marginBottom="10px"; row.innerHTML=`<div style="display:flex;gap:8px"><input class="bb-title" value="${escapeHtml(section.title||"")}"><button type="button" class="danger-btn bb-delete">Delete</button></div><textarea class="bb-text" rows="4" style="width:100%;margin-top:8px">${escapeHtml(section.text||"")}</textarea><label style="display:flex;gap:7px;align-items:center;margin-top:7px"><input class="bb-mandatory" type="checkbox" ${section.mandatoryQc?"checked":""}> Add this rule as mandatory QC</label>`; row.querySelector(".bb-delete").onclick=()=>row.remove(); els.brandBibleSections.appendChild(row); }
async function saveBrandBible(event){ event.preventDefault(); const data={}; [...els.brandBibleSections.children].forEach((row,i)=>{data[`section_${i+1}`]={title:row.querySelector(".bb-title").value.trim(),text:row.querySelector(".bb-text").value.trim(),mandatoryQc:row.querySelector(".bb-mandatory").checked}}); const ok=await write(`brandBibles/${brandKey(currentBrand)}`,data,"Brand Bible saved"); if(ok) els.brandBibleDialog.close(); }
function renderBrandBibleNotice(){ const box=document.querySelector("#brandRulesNotice"); if(!box)return; const rules=Object.values(getBrandBible()).filter(x=>x?.mandatoryQc&&x?.text); box.innerHTML=rules.length?`<div><strong>Brand Rules — ${escapeHtml(currentBrand)}</strong><p>${rules.map(x=>`• ${escapeHtml(x.text)}`).join("<br>")}</p></div>`:`<div><strong>Brand Rules</strong><p>No mandatory Brand Bible rules saved yet.</p></div>`; }
function mandatoryQcRules(){return Object.values(getBrandBible()).filter(x=>x?.mandatoryQc&&x?.text).map(x=>x.text)}
function addQcRow(text,done=false){ const row=document.createElement("div"); row.className="brand-management-item qc-row"; row.innerHTML=`<label style="display:flex;align-items:center;gap:8px;flex:1"><input type="checkbox" class="qc-done" ${done?"checked":""}><input class="qc-text" value="${escapeHtml(text)}"></label><button type="button" class="danger-btn qc-delete">Delete</button>`; row.querySelector(".qc-delete").onclick=()=>{row.remove();syncQualityGate()}; row.querySelector(".qc-done").onchange=syncQualityGate; els.qcChecklist.appendChild(row); }
function prepareQcEditor(item=null){ els.qcChecklist.innerHTML=""; let list=item?.qc?.length?item.qc:DEFAULT_INTERNAL_QC.map(text=>({text,done:false})); if(!item) mandatoryQcRules().forEach(text=>list.push({text,done:false})); list.forEach(q=>addQcRow(q.text||q,!!q.done)); document.querySelector("#creatorCheck").checked=!!item?.creatorCheck; document.querySelector("#internalReview").checked=!!item?.internalReview; document.querySelector("#nicoleApproval").checked=!!item?.nicoleApproval; document.querySelector("#revisionReason").value=item?.revisionReason||""; document.querySelector("#revisionOther").value=item?.revisionOther||""; ensureReelQc(); syncQualityGate(); }
function ensureReelQc(){ if(document.querySelector("#contentType")?.value!=="Reel")return; if(![...document.querySelectorAll(".qc-text")].some(x=>x.value==="Premium Music Vibes QC")) addQcRow("Premium Music Vibes QC",false); }
function readQcEditor(){return [...document.querySelectorAll(".qc-row")].map(row=>({text:row.querySelector(".qc-text").value.trim(),done:row.querySelector(".qc-done").checked})).filter(x=>x.text)}
function syncQualityGate(){ const qc=readQcEditor(); const ready=qc.length>0&&qc.every(x=>x.done)&&document.querySelector("#creatorCheck").checked&&document.querySelector("#internalReview").checked&&document.querySelector("#nicoleApproval").checked; document.querySelector("#readyForClient").checked=ready; }
function getSopResources(){return remote.sopResources?.[brandKey(currentBrand)]||{}}
function renderSopResources(){ if(!els.sopResourceList)return; const items=Object.entries(getSopResources()); els.sopResourceList.innerHTML=items.length?"":"<p>No SOP resources saved for this brand yet.</p>"; items.forEach(([id,item])=>{const row=document.createElement("div");row.className="brand-management-item";row.innerHTML=`<div><strong>${item.kind==="pdf"?"PDF":"Link"}: ${escapeHtml(item.title||"Untitled")}</strong><div style="color:var(--muted);margin-top:4px">${escapeHtml(item.notes||"")}</div>${item.url?`<a href="${escapeHtml(item.url)}" target="_blank" rel="noopener">Open resource</a>`:""}</div><button class="danger-btn">Delete</button>`;row.querySelector("button").onclick=async()=>{if(confirm("Delete this SOP resource?")) await write(`sopResources/${brandKey(currentBrand)}/${id}`,null,"SOP resource deleted")};els.sopResourceList.appendChild(row)})}
async function saveSopLink(event){event.preventDefault();const id=crypto.randomUUID();const payload={kind:"link",title:document.querySelector("#sopLinkTitle").value.trim(),url:document.querySelector("#sopLinkUrl").value.trim(),notes:document.querySelector("#sopLinkNotes").value.trim(),updatedAt:Date.now(),updatedBy:auth.currentUser.uid};const ok=await write(`sopResources/${brandKey(currentBrand)}/${id}`,payload,"SOP link saved");if(ok){event.target.reset();els.sopLinkDialog.close()}}
async function saveSopPdfs(event){const files=[...(event.target.files||[])];for(const file of files){if(file.type!=="application/pdf"&&!file.name.toLowerCase().endsWith(".pdf"))continue;if(file.size>2*1024*1024){alert(`${file.name} is over 2 MB and was skipped.`);continue}const dataUrl=await readFileAsDataUrl(file);const id=crypto.randomUUID();await write(`sopResources/${brandKey(currentBrand)}/${id}`,{kind:"pdf",title:file.name,notes:"",size:file.size,dataUrl,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},"SOP PDF saved")}event.target.value=""}
