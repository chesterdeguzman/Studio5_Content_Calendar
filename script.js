...
function openQcStage(itemId, stageKey) {
  ...
  context.innerHTML = `<strong>${escapeHtml(item.title || "Untitled content")}</strong><span>${escapeHtml(item.type || "Content")} · Week ${escapeHtml(item.week || "—")} · ${escapeHtml(item.assignee || "Unassigned")}</span><p><b>Goal:</b> ${escapeHtml(item.contentGoal || "—")}</p><p><b>Focus:</b> ${escapeHtml(item.productFocus || item.contentFocus || "—")}</p><p><b>Hook:</b> ${escapeHtml(item.hook || "—")}</p>`;
