...
import { getAuth, onAuthStateChanged, signInAnonymously, signOut } from "https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js";
import {
  getDatabase,
  ref,
  onValue,
...
async function ensureAnonymousAuth({ forceFresh = false } = {}) {
  if (forceFresh) {
    try { if (auth?.currentUser) await signOut(auth); } catch (error) { console.warn("Auth reset warning:", error); }
    const credential = await signInAnonymously(auth);
    return credential.user;
  }

  // Firebase restores persisted auth asynchronously. Give it a short chance to do so.
  ...
  if (!user) {
    try {
      user = await new Promise((resolve) => {
        let done = false;
        const timer = setTimeout(() => { if (!done) { done = true; unsub(); resolve(auth.currentUser); } }, 2500);
        const unsub = onAuthStateChanged(auth, (restored) => {
          if (!done) { done = true; clearTimeout(timer); unsub(); resolve(restored); }
        }, () => { if (!done) { done = true; clearTimeout(timer); unsub(); resolve(auth.currentUser); } });
      });
    } catch {
  ...
  // A persisted anonymous user can exist while its cached token is stale/revoked.
  // Force-refresh the token now. If that fails, transparently create a fresh session.
  try {
    await user.getIdToken(true);
    return user;
  } catch (error) {
    console.warn("Stored Firebase session was stale; refreshing automatically.", error);
...
async function init() {
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      // Do maintenance only after the live listener is attached.
      // If a seed/read is slow, the main interface can still load normally.
