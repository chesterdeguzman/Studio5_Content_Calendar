...
function bibleEntries(){
  ...
  else if(b.entries&&typeof b.entries==='object') entries=Object.values(b.entries);
  else {
    const legacy=getBrandResources()?.notes;
    if(legacy) entries=[{section:'Things to Check',text:legacy}];
  }
  return entries.filter(x=>x&&x.text).map(x=>({...x,section:normalizeBibleSection(x.section)}));
}
function getBibleCategories(){ return [...DEFAULT_BIBLE_SECTIONS]; }
function renderBrandBibleReminder(){
  const el=document.querySelector('#brandBibleReminder');
  if(!el)return;
  const entries=bibleEntries();
  el.classList.toggle('hidden',!entries.length);
  el.innerHTML=entries.length?`<strong>${escapeHtml(currentBrand)} Brand Bible</strong><span>${entries.length} saved note${entries.length===1?'':'s'} across Things to Check and Monthly Content Focus — automatically shown in Add Content.</span>`:'';
}
function openBrandBible(){
  document.querySelector('#brandBibleTitle').textContent=`${currentBrand} — Brand Bible`;
  renderBrandBibleWorkspace();
...
function renderBrandBibleWorkspace(){
  ...
  list.innerHTML='';
  const entries=bibleEntries();
  DEFAULT_BIBLE_SECTIONS.forEach(category=>{
    const group=document.createElement('section');
    group.className='bible-category-group';
    group.dataset.category=category;
    const actionLabel=category==='Things to Check'?'+ Add Rule':'+ Add Focus';
    const emptyText=category==='Things to Check'?'No checks saved yet.':'No monthly content focus saved yet.';
    group.innerHTML=`<div class="bible-category-head"><div><span class="eyebrow">Brand Bible</span><h4>${escapeHtml(category)}</h4></div><button type="button" class="ghost-btn bible-add-note">${actionLabel}</button></div><div class="bible-category-notes"></div>`;
    const notes=entries.filter(x=>x.section===category);
    const notesWrap=group.querySelector('.bible-category-notes');
    if(notes.length) notes.forEach(x=>addBibleNoteRow(category,x.text,notesWrap));
    else notesWrap.innerHTML=`<div class="bible-empty-note">${emptyText}</div>`;
    group.querySelector('.bible-add-note').addEventListener('click',()=>{
      notesWrap.querySelector('.bible-empty-note')?.remove();
      const row=addBibleNoteRow(category,'',notesWrap);
      row.querySelector('.bible-text')?.focus();
    });
    list.appendChild(group);
  });
}
function addBibleNoteRow(section,text='',target=null){
  const row=document.createElement('div');
  row.className='bible-entry';
  row.dataset.section=normalizeBibleSection(section);
  const isMonthly=row.dataset.section==='Monthly Content Focus';
  row.innerHTML=`<textarea class="bible-text" rows="5" placeholder="${isMonthly?'Add this month’s content focus. Press Enter for a new line.':'Add rules or checks here. Press Enter for a new line / separate rule.'}">${escapeHtml(text)}</textarea><button type="button" class="danger-btn bible-delete">Delete</button>`;
  row.querySelector('.bible-delete').addEventListener('click',()=>{
    const wrap=row.parentElement;
    row.remove();
    if(wrap && !wrap.querySelector('.bible-entry')) wrap.innerHTML=`<div class="bible-empty-note">${isMonthly?'No monthly content focus saved yet.':'No checks saved yet.'}</div>`;
  });
  (target||document.querySelector('#brandBibleList')).appendChild(row);
  return row;
}
function addBibleEntryRow(section='Things to Check',text=''){
  const normalized=normalizeBibleSection(section);
  const group=[...document.querySelectorAll('.bible-category-group')].find(x=>x.dataset.category===normalized);
  const wrap=group?.querySelector('.bible-category-notes');
  if(wrap){ wrap.querySelector('.bible-empty-note')?.remove(); return addBibleNoteRow(normalized,text,wrap); }
  return null;
}
async function saveBrandBible(e){
  e.preventDefault();
  const rows=[...document.querySelectorAll('#brandBibleDialog .bible-entry')];
  const entries=rows.map(r=>({section:normalizeBibleSection(r.dataset.section),text:r.querySelector('.bible-text').value.trim()})).filter(x=>x.text);
  const ok=await write(`brandBibles/${brandKey(currentBrand)}`,{categories:[...DEFAULT_BIBLE_SECTIONS],entries,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Bible saved');
  if(ok) document.querySelector('#brandBibleDialog').close();
}
function bibleTextLines(text){
  return String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
}
function renderContentBrandRules(){
  const el=document.querySelector('#contentBrandRules');
  if(!el)return;
  const entries=bibleEntries();
  if(!entries.length){
    el.innerHTML=`<div class="content-rules-head"><strong>${escapeHtml(currentBrand)} — no Brand Bible notes saved yet</strong><span>Ask for clarification rather than assume.</span></div>`;
    return;
  }
  const sections=DEFAULT_BIBLE_SECTIONS.map(section=>({section,lines:entries.filter(x=>x.section===section).flatMap(x=>bibleTextLines(x.text))})).filter(x=>x.lines.length);
  el.innerHTML=`<div class="content-rules-head"><strong>${escapeHtml(currentBrand)} — Brand Bible</strong><span>Automatic Brand Bible reminder</span></div><div class="content-rules-sections">${sections.map(group=>`<section class="content-rule-section"><strong>${escapeHtml(group.section)}</strong><ul>${group.lines.map(line=>`<li>${escapeHtml(line)}</li>`).join('')}</ul></section>`).join('')}</div>`;
}
