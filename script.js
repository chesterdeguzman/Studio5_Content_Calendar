...
let remote = { brands: {}, targets: DEFAULT_TARGETS, calendar: {}, specialDays: {}, assetRequests: {}, brandResources: {}, brandBibles: {}, brandMaterials: {}, sopResources: {}, contentLinks: {} };
...
function getBrandMaterials(brand = currentBrand) {
  return remote.brandMaterials?.[brandKey(brand)] || {};
}

function getBrandResourceTypes(brand = currentBrand) {
  const data = getBrandMaterials(brand);
...
function getOtherResourcesForBrand(brand = currentBrand) {
  const data = getBrandMaterials(brand);
  ...
  })).filter((item) => item.url || item.notes || item.title !== "Resource");
}

function otherResourcesPlainText(brand = currentBrand) {
  const resources = getOtherResourcesForBrand(brand);
  if (!resources.length) return "";
  return `Other Materials / Resources:\n${resources.map((item, index) => `${index + 1}. ${item.title} [${item.type}]${item.url ? `: ${item.url}` : ""}${item.notes ? ` — ${item.notes}` : ""}`).join("\n")}`;
}
...
function brandBiblePlainText(brand = currentBrand) {
  ...
  const resourcesText = otherResourcesPlainText(brand);
  return notesText + linksText + (resourcesText ? `\n\n${resourcesText}` : "");
}
...
function brandBibleHtmlBlock(brand = currentBrand) {
  ...
  const links = getBrandBibleLinksForBrand(brand);
  const resources = getOtherResourcesForBrand(brand);
  if (!entries.length && !links.length && !resources.length) return "";
  const grouped = new Map();
  entries.forEach((entry) => {
    if (!grouped.has(entry.section)) grouped.set(entry.section, []);
  ...
  const resourcesHtml = resources.length ? `<div class="bible-export-section"><b>Other Materials / Resources</b><ul>${resources.map((item) => `<li><strong>${escapeHtml(item.title)}</strong> <em>(${escapeHtml(item.type)})</em>${item.url ? ` — <a href="${escapeHtml(item.url)}">${escapeHtml(item.url)}</a>` : ""}${item.notes ? `<br><span>${escapeHtml(item.notes)}</span>` : ""}</li>`).join("")}</ul></div>` : "";
  return `<div class="brand-notes-box"><strong>Brand Bible / Notes</strong>${notesHtml}${linksHtml}${resourcesHtml}</div>`;
}
...
async function exportPdf() {
  ...
  try {
    ...
    const otherResources = getOtherResourcesForBrand();
    if (otherResources.length) {
      ensureSpace(12 + otherResources.length * 10);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.setTextColor(50);
      doc.text("Other Materials / Resources", margin, y);
      y += 6;
      otherResources.forEach((item, index) => {
        ensureSpace(12);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(8.8);
        doc.setTextColor(45);
        const heading = pdfSafeText(`${index + 1}. ${item.title} (${item.type})`);
        doc.text(heading, margin + 2, y);
        y += 4.5;
        if (item.url) {
          const urlLabel = pdfSafeText(item.url);
          doc.setFont("helvetica", "normal");
          doc.setTextColor(30, 85, 180);
          const linkLines = doc.splitTextToSize(urlLabel, contentWidth - 4);
          doc.textWithLink(linkLines[0], margin + 4, y, { url: item.url });
          if (linkLines.length > 1) {
            doc.setTextColor(95);
            doc.text(linkLines.slice(1), margin + 4, y + 4.1);
          }
          y += Math.max(4.5, linkLines.length * 4.1);
        }
        if (item.notes) {
          doc.setFont("helvetica", "normal");
          doc.setTextColor(95);
          const noteLines = doc.splitTextToSize(pdfSafeText(item.notes), contentWidth - 4);
...
function renderOtherResourcesView(){
  ...
  const items=getOtherResourcesForBrand();
  const customRaw=getBrandMaterials().types;
  ...
  summary.innerHTML=`<div class="resource-view-stat"><span>Selected Brand</span><strong>${escapeHtml(currentBrand)}</strong></div><div class="resource-view-stat"><span>Saved Resources</span><strong>${items.length}</strong></div><div class="resource-view-stat"><span>Custom Types</span><strong>${custom.length}</strong></div>`;
  if(!items.length){
    list.innerHTML='<div class="other-resources-view-empty"><strong>No other resources saved yet.</strong><p>Use “+ Add / Manage Resources” to add a content tracker, marketing planner, Google Sheet, PDF, folder, dashboard or another useful material for this brand.</p></div>';
    return;
  }
  list.innerHTML='';
  items.forEach(item=>{
...
function openOtherResources(){
  document.querySelector('#otherResourcesTitle').textContent=`${currentBrand} — Other Resources`;
...
function resourceTypeOptions(selected='Other'){
  const types=getBrandResourceTypes();
...
function renderOtherResourceTypes(){
  ...
  const stored=getBrandMaterials().types;
...
async function saveOtherResources(e){
  ...
  const ok=await write(`brandMaterials/${brandKey(currentBrand)}`,{types,resources,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Other resources saved');
