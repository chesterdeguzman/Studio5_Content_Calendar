...
const els = {
  ...
  qcDialog: document.querySelector("#qcDialog"),
  qcForm: document.querySelector("#qcForm"),
  closeQcDialogBtn: document.querySelector("#closeQcDialogBtn"),
  cancelQcDialogBtn: document.querySelector("#cancelQcDialogBtn"),
  qcContentContext: document.querySelector("#qcContentContext"),
  qcBrandRules: document.querySelector("#qcBrandRules"),
...
function bindEvents() {
  ...
  els.qcForm?.addEventListener("submit", saveQcReview);
  els.closeQcDialogBtn?.addEventListener("click", closeQcDialog);
  els.cancelQcDialogBtn?.addEventListener("click", closeQcDialog);
...
function defaultQcState(type = "") {
  ...
  return { items: labels.map((label) => ({ label, checked: false })), creatorCheck: false, internalReview: false, leadApproval: false, failureReason: "", readyForClient: false };
}

function qcStageKey(item) {
  const q = item?.qc || {};
  if (q.failureReason) return "revision";
  if (q.readyForClient) return "ready";
  if (!q.creatorCheck) return "creator";
  if (!q.internalReview) return "internal";
  if (!q.leadApproval) return "lead";
  return "lead";
}

function qcStageLabel(item) {
  return { creator: "Creator Check", internal: "Internal Review", lead: "Nicole Approval", ready: "Ready for Client", revision: "Needs Revision" }[qcStageKey(item)] || "Creator Check";
}
...
function renderQualityControl() {
  ...
  }).sort((a, b) => (a.qc?.readyForClient === b.qc?.readyForClient ? 0 : a.qc?.readyForClient ? 1 : -1) || Number(a.week) - Number(b.week) || (a.date || "").localeCompare(b.date || ""));

  const counts = { creator: 0, internal: 0, lead: 0, ready: 0, revision: 0 };
  items.forEach((item) => { counts[qcStageKey(item)] = (counts[qcStageKey(item)] || 0) + 1; });
  if (els.qcSummaryMini) els.qcSummaryMini.innerHTML = `<span>${counts.creator} creator</span><span>${counts.internal} internal</span><span>${counts.lead} Nicole</span><span>${counts.ready} ready</span>`;
  ...
  filtered.forEach((item) => {
    const q = item.qc || defaultQcState(item.type);
    const total = Array.isArray(q.items) ? q.items.length : Object.keys(q.items || {}).length;
    const checked = Array.isArray(q.items) ? q.items.filter((x) => x?.checked).length : Object.values(q.items || {}).filter((x) => x?.checked).length;
    const card = document.createElement("article");
    card.className = "qc-queue-card";
    card.dataset.stage = qcStageKey(item);
    card.innerHTML = `<div class="qc-card-main"><div class="qc-card-top"><span class="type-badge" data-type="${escapeHtml(item.type || "")}">${escapeHtml(item.type || "Content")}</span><span class="qc-stage-badge" data-stage="${escapeHtml(qcStageKey(item))}">${escapeHtml(qcStageLabel(item))}</span></div><h4>${escapeHtml(item.title || "Untitled content")}</h4><p>${escapeHtml(item.productFocus || item.contentFocus || item.contentGoal || "No strategy focus added.")}</p><div class="qc-card-meta"><span>Week ${escapeHtml(item.week || "—")}</span><span>${escapeHtml(item.date ? formatDate(item.date) : "No date")}</span><span>${escapeHtml(item.assignee || "Unassigned")}</span></div></div><div class="qc-card-progress"><strong>${checked}/${total || 0}</strong><small>QC checks</small><div class="progress"><span style="width:${percent(checked, total || 1)}"></span></div>${q.failureReason ? `<div class="qc-failure-note">${escapeHtml(q.failureReason)}</div>` : ""}<button type="button" class="primary-btn qc-review-btn">Review QC</button></div>`;
    card.querySelector(".qc-review-btn").addEventListener("click", () => openQcDialog(item.id));
    els.qcQueue.appendChild(card);
  });
}

function renderQcBrandRules() {
  if (!els.qcBrandRules) return;
  const entries = bibleEntries();
  if (!entries.length) {
    els.qcBrandRules.innerHTML = '<strong>Brand Bible</strong><p>No Brand Bible rules saved yet for this brand.</p>';
    return;
  }
  els.qcBrandRules.innerHTML = `<div class="content-rules-head"><strong>${escapeHtml(currentBrand)} Brand Bible</strong><span>Review before approving</span></div>${entries.map((entry) => `<div><strong>${escapeHtml(entry.category || "Brand Rule")}:</strong> ${escapeHtml(entry.note || entry.text || "")}</div>`).join("")}`;
}

function openQcDialog(id) {
  const item = getMonthItems().find((entry) => entry.id === id);
  if (!item || !els.qcDialog) return;
  editingQcId = id;
  document.querySelector("#qcDialogTitle").textContent = item.title || "Review Content";
  if (els.qcContentContext) {
    els.qcContentContext.innerHTML = `<div><span>Content Goal</span><strong>${escapeHtml(item.contentGoal || "—")}</strong></div><div><span>Product / Service Focus</span><strong>${escapeHtml(item.productFocus || item.contentFocus || "—")}</strong></div><div><span>Hook</span><strong>${escapeHtml(item.hook || "—")}</strong></div><div><span>Why should they care?</span><strong>${escapeHtml(item.audienceBenefit || "—")}</strong></div><div><span>Content Direction</span><strong>${escapeHtml(item.contentFocus || "—")}</strong></div><div><span>CTA</span><strong>${escapeHtml(item.cta || "—")}</strong></div><div><span>Required Assets</span><strong>${escapeHtml(item.requiredAssets || "—")}</strong></div><div><span>Do Not Use / Avoid</span><strong>${escapeHtml(item.avoidContent || "—")}</strong></div>`;
  }
  renderQcBrandRules();
  loadQualityGate(item);
  els.qcDialog.showModal();
}

function closeQcDialog() {
  els.qcDialog?.close();
  editingQcId = null;
}

async function saveQcReview(event) {
  event.preventDefault();
  if (!editingQcId) return;
  const qc = collectQualityGate();
  const itemRef = ref(db, `${ROOT_PATH}/calendar/${brandKey(currentBrand)}/${currentMonth}/${editingQcId}`);
  setSyncStatus("saving", "Saving QC...");
  try {
    await update(itemRef, { qc, updatedAt: Date.now(), updatedBy: auth.currentUser.uid });
    await update(ref(db, `${ROOT_PATH}/meta`), { updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    setSyncStatus("saved", "QC saved");
    closeQcDialog();
  } catch (error) {
    console.error(error);
    setSyncStatus("error", readableFirebaseError(error));
...
function addBibleEntryRow(section='Brand Rules',text=''){ const row=document.createElement('div'); row.className='bible-entry'; row.innerHTML=`<select class="bible-section">${DEFAULT_BIBLE_SECTIONS.map(x=>`<option ${x===section?'selected':''}>${escapeHtml(x)}</option>`).join('')}</select><textarea class="bible-text" rows="3" placeholder="Add a specific client rule, preference or instruction...">${escapeHtml(text)}</textarea><button type="button" class="danger-btn bible-delete">Delete</button>`; row.querySelector('.bible-delete').onclick=()=>row.remove(); document.querySelector('#brandBibleList').appendChild(row); }
async function saveBrandBible(e){ e.preventDefault(); const entries=[...document.querySelectorAll('.bible-entry')].map(r=>({section:r.querySelector('.bible-section').value,text:r.querySelector('.bible-text').value.trim()})).filter(x=>x.text); await write(`brandBibles/${brandKey(currentBrand)}`,{entries,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Bible saved'); document.querySelector('#brandBibleDialog').close(); }
...
function addQcRow(label='',checked=false,locked=false){ const row=document.createElement('div');row.className='qc-row';row.innerHTML=`<label><input type="checkbox" class="qc-check" ${checked?'checked':''}><input type="text" class="qc-label" value="${escapeHtml(label)}" placeholder="QC check..." ${locked?'data-default="1"':''}></label><button type="button" class="danger-btn qc-delete">Delete</button>`;row.querySelector('.qc-delete').onclick=()=>{row.remove();updateQualityGateState();};row.querySelector('.qc-check').onchange=updateQualityGateState;document.querySelector('#qcChecklist').appendChild(row); }
function resetQualityGate(){ const list=document.querySelector('#qcChecklist'); if(!list)return;list.innerHTML='';DEFAULT_QC.forEach(x=>addQcRow(x)); document.querySelector('#creatorCheck').checked=false;document.querySelector('#internalReview').checked=false;document.querySelector('#leadApproval').checked=false;document.querySelector('#qcFailureReason').value='';document.querySelector('#readyForClientBtn').dataset.ready='false'; ensureReelQc(); updateQualityGateState(); }
function ensureReelQc(){ const list=document.querySelector('#qcChecklist'); if(!list)return; const qcItem=editingQcId?getMonthItems().find((entry)=>entry.id===editingQcId):null; const reel=(qcItem?.type || document.querySelector('#contentType')?.value)==='Reel'; const labels=[...list.querySelectorAll('.qc-label')].map(x=>x.value); if(reel)REEL_QC.forEach(x=>{if(!labels.includes(x))addQcRow(x)}); updateQualityGateState(); }
function loadQualityGate(item){ const q=item.qc||{}; const list=document.querySelector('#qcChecklist');list.innerHTML=''; const rows=Array.isArray(q.items)?q.items:(q.items&&typeof q.items==='object'?Object.values(q.items):DEFAULT_QC.map(label=>({label,checked:false}))); rows.forEach(x=>addQcRow(x.label||'',!!x.checked)); document.querySelector('#creatorCheck').checked=!!q.creatorCheck;document.querySelector('#internalReview').checked=!!q.internalReview;document.querySelector('#leadApproval').checked=!!q.leadApproval;document.querySelector('#qcFailureReason').value=q.failureReason||'';document.querySelector('#readyForClientBtn').dataset.ready=q.readyForClient?'true':'false';ensureReelQc();updateQualityGateState(); }
function collectQualityGate(){ return {items:[...document.querySelectorAll('.qc-row')].map(r=>({label:r.querySelector('.qc-label').value.trim(),checked:r.querySelector('.qc-check').checked})).filter(x=>x.label),creatorCheck:document.querySelector('#creatorCheck').checked,internalReview:document.querySelector('#internalReview').checked,leadApproval:document.querySelector('#leadApproval').checked,failureReason:document.querySelector('#qcFailureReason').value,readyForClient:document.querySelector('#readyForClientBtn').dataset.ready==='true'}; }
function gateComplete(){ const rows=[...document.querySelectorAll('.qc-row')];return rows.length>0&&rows.every(r=>r.querySelector('.qc-check').checked)&&document.querySelector('#creatorCheck').checked&&document.querySelector('#internalReview').checked&&document.querySelector('#leadApproval').checked; }
function updateQualityGateState(){ const btn=document.querySelector('#readyForClientBtn'),msg=document.querySelector('#qualityGateMessage');if(!btn)return; const complete=gateComplete();btn.disabled=!complete; if(btn.dataset.ready==='true'&&!complete)btn.dataset.ready='false';btn.textContent=btn.dataset.ready==='true'?'✓ Ready for Client':'Ready for Client';msg.textContent=complete?'Quality Gate complete. You can mark this content Ready for Client.':'Complete every QC item, Creator Check, Internal Review and Nicole / Lead Approval to unlock Ready for Client.'; }
function markReadyForClient(){if(!gateComplete())return;const btn=document.querySelector('#readyForClientBtn');btn.dataset.ready='true';btn.textContent='✓ Ready for Client';document.querySelector('#qualityGateMessage').textContent='Quality Gate passed — ready to send to the client.';}
document.addEventListener('change',e=>{if(['creatorCheck','internalReview','leadApproval'].includes(e.target?.id))updateQualityGateState();});
