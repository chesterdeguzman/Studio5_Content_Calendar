...
function buildCard(item) {
  ...
  if (item.qc?.readyForClient) node.querySelector(".card-title").insertAdjacentHTML("afterend", `<span class="ready-pill">Ready for Client</span>`);
...
function bindEvents() {
  ...
  document.querySelectorAll(".nav-item").forEach((button) => button.addEventListener("click", () => {
    ...
    document.querySelector("#pageTitle").textContent = { calendar: "Monthly Content Calendar", special: "Special Days Calendar", overview: "Brand Overview", links: "Content Links", usecase: "Use Case Diagram", sop: "SOP Library", settings: "Settings" }[button.dataset.view];
  }));
...
function ensureReelQc(){ const list=document.querySelector('#qcChecklist'); if(!list)return; const reel=document.querySelector('#contentType').value==='Reel'; const labels=[...list.querySelectorAll('.qc-label')].map(x=>x.value); if(reel)REEL_QC.forEach(x=>{if(!labels.includes(x))addQcRow(x)}); updateQualityGateState(); }
