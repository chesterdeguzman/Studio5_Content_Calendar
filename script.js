...
function renderBrandBibleWorkspace(){
  ...
  getBibleCategories().forEach(category=>{
    const group=document.createElement('section');
    group.className='bible-category-group';
    group.dataset.category=category;
    group.innerHTML=`<div class="bible-category-head"><div><span class="eyebrow">Brand Bible Category</span><h4>${escapeHtml(category)}</h4></div><button type="button" class="ghost-btn bible-add-note">+ Add Note</button></div><div class="bible-category-notes"></div>`;
    const notes=entries.filter(x=>String(x?.section||'')===category);
    ...
    else notesWrap.innerHTML='<div class="bible-empty-note">No notes saved in this category yet.</div>';
...
function addBibleNoteRow(section,text='',target=null){
  ...
  row.innerHTML=`<div class="bible-note-label"><strong>${escapeHtml(section)}</strong><span>Note</span></div><textarea class="bible-text" rows="4" placeholder="Add a specific client rule, preference or instruction...">${escapeHtml(text)}</textarea><button type="button" class="danger-btn bible-delete">Delete</button>`;
  row.querySelector('.bible-delete').addEventListener('click',()=>{
    const wrap=row.parentElement;
    row.remove();
    if(wrap && !wrap.querySelector('.bible-entry')) wrap.innerHTML='<div class="bible-empty-note">No notes saved in this category yet.</div>';
  });
  (target||document.querySelector('#brandBibleList')).appendChild(row);
  return row;
}
function addBibleEntryRow(section='Brand Rules',text=''){
  const group=[...document.querySelectorAll('.bible-category-group')].find(x=>x.dataset.category===section);
  const wrap=group?.querySelector('.bible-category-notes');
  if(wrap){ wrap.querySelector('.bible-empty-note')?.remove(); return addBibleNoteRow(section,text,wrap); }
  return null;
}
function addBrandBibleCategory(){
  ...
  remote.brandBibles[brandKey(currentBrand)]={...bible,categories:[...new Set([...saved,clean])]};
  renderBrandBibleWorkspace();
  const group=[...document.querySelectorAll('.bible-category-group')].find(x=>x.dataset.category===clean);
  group?.scrollIntoView({behavior:'smooth',block:'center'});
}
async function saveBrandBible(e){
  e.preventDefault();
  const rows=[...document.querySelectorAll('.bible-entry')];
  const entries=rows.map(r=>({section:r.dataset.section||'Other Notes',text:r.querySelector('.bible-text').value.trim()})).filter(x=>x.text);
  const current=getBrandBible();
  const saved=Array.isArray(current.categories) ? current.categories : (current.categories&&typeof current.categories==='object' ? Object.values(current.categories) : []);
  const categories=[...new Set([...DEFAULT_BIBLE_SECTIONS,...saved,...entries.map(x=>x.section)].map(x=>String(x||'').trim()).filter(Boolean))];
  const ok=await write(`brandBibles/${brandKey(currentBrand)}`,{categories,entries,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Bible saved');
