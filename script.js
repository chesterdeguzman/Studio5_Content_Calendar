...
import { initializeAuth, inMemoryPersistence, signInAnonymously, signOut } from "https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js";
import {
  getDatabase,
  goOnline,
  ref,
...
import { AI_BACKEND_URL } from "./ai-config.js";
...
let remote = { brands: {}, targets: DEFAULT_TARGETS, calendar: {}, specialDays: {}, assetRequests: {}, brandResources: {}, brandBibles: {}, brandMaterials: {}, studioResources: {}, sopResources: {}, contentLinks: {}, qcLeadReminders: {} };
...
let hasRetriedRealtimeAuth = false;
...
const els = {
  ...
  assetRequestFromRole: document.querySelector("#assetRequestFromRole"),
  assetRequestToRole: document.querySelector("#assetRequestToRole"),
  assetReplySection: document.querySelector("#assetReplySection"),
  assetReplyThread: document.querySelector("#assetReplyThread"),
  assetReplyRole: document.querySelector("#assetReplyRole"),
  assetReplyMessage: document.querySelector("#assetReplyMessage"),
  assetReplyBtn: document.querySelector("#assetReplyBtn"),
...
queueMicrotask(startAccessGate);
...
async function ensureAnonymousAuth({ forceFresh = false } = {}) {
  // IMPORTANT: Firebase Auth is intentionally memory-only for this app.
  // The Studio 5 workspace login is remembered separately for 30 days, but
  // Firebase receives a fresh anonymous identity on every browser reload.
  // This automatically does what signing out/in used to do manually and
  // prevents a stale anonymous token from leaving the dashboard on Connecting.
  if (forceFresh && auth?.currentUser) {
    try { await withTimeout(signOut(auth), 5000, "Firebase sign-out"); }
    catch (error) { console.warn("Firebase session reset warning:", error); }
  }

  if (auth?.currentUser && !forceFresh) return auth.currentUser;
  const credential = await withTimeout(signInAnonymously(auth), 12000, "Firebase anonymous sign-in");
  return credential.user;
}

async function loadInitialSnapshot() {
  const snapshot = await withTimeout(get(rootRef), 12000, "Firebase database read");
...
async function init() {
  ...
  initPromise = (async () => {
    try {
      ...
      // Do NOT persist Firebase's anonymous user between page loads. The
      // workspace gate handles the 30-day login separately.
      ...
      // Load once immediately so the brand dropdown, content, QC and other
      // features appear even before the realtime stream has settled.
      await loadInitialSnapshot();
      connectRealtime();

      // Maintenance is intentionally background-only and cannot block startup.
      ensureInitialData().then(() => ensureBrandSpecialDays2026()).catch((error) => {
        console.warn("Background Firebase maintenance warning:", error);
      });
    } catch (error) {
      console.error("Firebase startup error:", error);
      connected = false;
      setSyncStatus("error", readableFirebaseError(error));
      // Render the local shell/default brands instead of leaving a blank UI.
      try { renderAll(); } catch (renderError) { console.error("Render error:", renderError); }
      initPromise = null;
    }
  })();
...
function connectRealtime() {
  ...
  // If the listener never produces its first snapshot, automatically refresh auth once.
  connectWatchdog = setTimeout(() => {
    if (!connected && !hasRetriedRealtimeAuth) {
      hasRetriedRealtimeAuth = true;
      recoverFirebaseSession("initial connection timeout").catch((error) => {
  ...
  unsubscribeRoot = onValue(
    ...
    (error) => {
      ...
      const looksAuthRelated = code.includes("permission-denied") || code.includes("auth") || message.includes("permission") || message.includes("token");

      if (looksAuthRelated && !hasRetriedRealtimeAuth) {
        hasRetriedRealtimeAuth = true;
        recoverFirebaseSession(error?.code || error?.message).catch((recoveryError) => {
          console.error("Automatic Firebase auth recovery failed:", recoveryError);
          setSyncStatus("error", readableFirebaseError(recoveryError));
        });
        return;
      }
...
function normalizeRemote(value) {
  ...
  return {
    ...
    brandMaterials: data.brandMaterials && typeof data.brandMaterials === "object" ? data.brandMaterials : {},
    studioResources: data.studioResources && typeof data.studioResources === "object" ? data.studioResources : {},
    sopResources: data.sopResources && typeof data.sopResources === "object" ? data.sopResources : {},
    qcLeadReminders: data.qcLeadReminders && typeof data.qcLeadReminders === "object" ? data.qcLeadReminders : {},
    meta: data.meta || {}
  };
}
...
function renderAll() {
  ...
  renderSopResources();
  renderOtherResourcesView();
  renderQcWorkspace();
  renderQcLeadReminders();
}
...
function reviewHistoryArray(item) {
  const raw = item?.reviewHistory;
  if (Array.isArray(raw)) return raw.filter(Boolean);
  if (raw && typeof raw === "object") return Object.values(raw).filter(Boolean).sort((a, b) => Number(a?.at || 0) - Number(b?.at || 0));
  return [];
}

function reviewEventLabel(event) {
  return ({ Rejected: "Rejected", Revise: "Revision requested", Pending: "Sent for review", Approved: "Approved" })[event?.status] || event?.status || "Update";
}

function renderClientReviewHistory(item = null) {
  const wrap = document.querySelector("#clientReviewHistory");
  const badge = document.querySelector("#reviewStateBadge");
  if (!wrap || !badge) return;
  const status = item?.status || document.querySelector("#contentStatus")?.value || "Pending";
  badge.textContent = status;
  badge.dataset.status = status;
  const events = reviewHistoryArray(item);
  if (!events.length) {
    wrap.innerHTML = '<div class="review-empty">No review activity yet. Rejections, revision requests and resubmissions will appear here with date and time.</div>';
    return;
  }
  wrap.innerHTML = events.slice().reverse().map((event) => {
    const when = event.at ? new Date(Number(event.at)).toLocaleString() : "Time unavailable";
    return `<article class="review-event" data-status="${escapeHtml(event.status || "Pending")}"><div class="review-event-top"><strong>${escapeHtml(reviewEventLabel(event))}</strong><time>${escapeHtml(when)}</time></div>${event.comment ? `<p>${escapeHtml(event.comment)}</p>` : ""}${event.round ? `<small>Review round ${escapeHtml(event.round)}</small>` : ""}</article>`;
  }).join("");
}

async function sendForReviewAgain() {
  if (!editingId) return alert("Save this content first before sending it for review again.");
  const item = getMonthItems().find((entry) => entry.id === editingId);
  if (!item) return;
  const comment = document.querySelector("#contentNotes")?.value.trim() || "Updated content submitted for another review.";
  const history = reviewHistoryArray(item);
  const round = Math.max(1, ...history.map((event) => Number(event.round) || 1)) + 1;
  history.push({ status: "Pending", comment, at: Date.now(), round, action: "resubmitted", by: auth?.currentUser?.uid || "" });
  const itemRef = ref(db, `${ROOT_PATH}/calendar/${brandKey(currentBrand)}/${currentMonth}/${editingId}`);
  setSyncStatus("saving", "Sending for review…");
  try {
    await update(itemRef, { status: "Pending", notes: comment, reviewHistory: history, reviewRound: round, updatedAt: Date.now(), updatedBy: auth.currentUser.uid });
    document.querySelector("#contentStatus").value = "Pending";
    renderClientReviewHistory({ ...item, status: "Pending", reviewHistory: history });
    setSyncStatus("saved", "Sent for review again");
  } catch (error) {
    console.error(error);
    setSyncStatus("error", readableFirebaseError(error));
  }
...
function openNewDialog(week = 1) {
  ...
  setContentCreationMode("ai");
  refreshAiSourceSummary();
  document.querySelector("#aiContentIdea").value = "";
  setAiGenerationStatus("");
  renderClientReviewHistory(null);
  document.querySelector("#sendForReviewAgainBtn")?.classList.add("hidden");
...
function openEditDialog(id) {
  ...
  renderContentBrandRules();
  setContentCreationMode("manual");
  refreshAiSourceSummary();
  setAiGenerationStatus("");
  renderClientReviewHistory(item);
  document.querySelector("#sendForReviewAgainBtn")?.classList.remove("hidden");
  els.contentDialog.showModal();
}
...
async function saveContentFromForm(event) {
  ...
  const existingItem = editingId ? getMonthItems().find((entry) => entry.id === editingId) : null;
  const selectedStatus = document.querySelector("#contentStatus").value;
  const reviewHistory = reviewHistoryArray(existingItem);
  if (existingItem && selectedStatus !== (existingItem.status || "Pending")) {
    reviewHistory.push({
      status: selectedStatus,
      comment: document.querySelector("#contentNotes").value.trim(),
      at: Date.now(),
      round: Number(existingItem.reviewRound) || 1,
  ...
  const item = {
    ...
    reviewHistory,
    reviewRound: Number(existingItem?.reviewRound) || 1,
    updatedAt: Date.now(),
    updatedBy: auth.currentUser.uid
  };
...
async function updateStatus(id, status) {
  ...
  if (!item) return;
  let comment = "";
  if (["Rejected", "Revise"].includes(status)) {
    comment = window.prompt(status === "Rejected" ? "Add a rejection comment for the client notes:" : "Add revision feedback for the client notes:", item.notes || "") ?? "";
    if (!comment.trim()) return alert("Please add a comment so the team knows what needs to change.");
    comment = comment.trim();
  }
  const history = reviewHistoryArray(item);
  history.push({ status, comment: comment || item.notes || "", at: Date.now(), round: Number(item.reviewRound) || 1, action: "status-change", by: auth?.currentUser?.uid || "" });
  const itemRef = ref(db, `${ROOT_PATH}/calendar/${brandKey(currentBrand)}/${currentMonth}/${id}`);
  setSyncStatus("saving", "Saving…");
  try {
    await update(itemRef, { status, notes: comment || item.notes || "", reviewHistory: history, reviewRound: Number(item.reviewRound) || 1, updatedAt: Date.now(), updatedBy: auth.currentUser.uid });
    setSyncStatus("saved", "Saved online");
  } catch (error) {
    console.error(error);
    setSyncStatus("error", readableFirebaseError(error));
...
async function removeBrand(brand) {
  ...
  try {
    ...
    await update(ref(db, ROOT_PATH), {
      ...
      [`brandMaterials/${key}`]: null,
      [`contentLinks/${key}`]: null,
      [`qcLeadReminders/${key}`]: null,
  ...
  reader.onload = async () => {
    try {
      ...
      await set(rootRef, {
        ...
        brandMaterials: imported.brandMaterials || {},
        studioResources: imported.studioResources || {},
        sopResources: imported.sopResources || {},
        qcLeadReminders: imported.qcLeadReminders || {},
...
function bindEvents() {
  ...
  document.querySelectorAll(".nav-item").forEach((button) => button.addEventListener("click", () => {
    ...
    document.querySelector("#pageTitle").textContent = { calendar: "Monthly Content Calendar", qc: "Quality Control", qcreminders: "QC Lead Reminders", special: "Special Days Calendar", overview: "Brand Overview", links: "Content Links", usecase: "Use Case Diagram", sop: "Brand Brief", resources: "Other Resources", settings: "Settings" }[button.dataset.view];
  }));
  ...
  els.assetReplyBtn?.addEventListener("click", sendAssetReply);
  document.querySelector("#sendForReviewAgainBtn")?.addEventListener("click", sendForReviewAgain);
  ...
  document.querySelector("#addBrandBibleLinkBtn")?.addEventListener("click", () => addBrandBibleLinkRow());
  document.querySelector("#manageOtherResourcesBtn")?.addEventListener("click", openOtherResources);
  document.querySelector("#otherResourcesForm")?.addEventListener("submit", saveOtherResources);
  document.querySelector("#closeOtherResourcesBtn")?.addEventListener("click", closeOtherResources);
  document.querySelector("#cancelOtherResourcesBtn")?.addEventListener("click", closeOtherResources);
  document.querySelector("#addOtherResourceBtn")?.addEventListener("click", () => addOtherResourceRow());
  document.querySelector("#addResourceTypeBtn")?.addEventListener("click", addResourceTypeRow);
  ...
  document.querySelector("#aiModeBtn")?.addEventListener("click", () => setContentCreationMode("ai"));
  document.querySelector("#manualModeBtn")?.addEventListener("click", () => setContentCreationMode("manual"));
  document.querySelector("#generateContentAiBtn")?.addEventListener("click", generateContentWithAi);
  document.querySelector("#generateAnotherAiBtn")?.addEventListener("click", generateContentWithAi);
  document.querySelector("#readyForClientBtn")?.addEventListener("click", markReadyForClient);
  document.querySelector("#addQcReminderBtn")?.addEventListener("click", () => addQcLeadReminderCard());
  bindQcWorkspaceEvents();
}
...
function getBrandBibleEntriesForBrand(brand = currentBrand) {
  return bibleEntries(brand).map((entry) => ({
...
function getBrandBibleLinksForBrand(brand = currentBrand) {
  const bible = remote.brandBibles?.[brandKey(brand)] || {};
  const raw = Array.isArray(bible.links)
    ? bible.links
    : (bible.links && typeof bible.links === "object" ? Object.values(bible.links) : []);
  return raw.map((item) => ({
    label: String(item?.label || "Brand Brief").trim() || "Brand Brief",
    url: String(item?.url || "").trim()
  })).filter((item) => item.url);
}

const DEFAULT_RESOURCE_TYPES = ["Google Sheet", "Google Doc", "PDF", "Content Tracker", "Marketing Planner", "Folder / Drive", "Website", "Other"];

function getStudioResourcesData() {
  const data = remote.studioResources && typeof remote.studioResources === "object" ? remote.studioResources : {};
  const hasSavedStudioData = Object.keys(data).length > 0;
  if (hasSavedStudioData) return data;

  // Backwards compatibility for v27-v29. Until the Studio 5 library is saved,
  // show legacy resources that may have been stored under individual brands.
  const legacyResources = [];
  const legacyTypes = [];
  for (const legacy of Object.values(remote.brandMaterials || {})) {
    if (!legacy || typeof legacy !== "object") continue;
    const resources = Array.isArray(legacy.resources) ? legacy.resources : (legacy.resources && typeof legacy.resources === "object" ? Object.values(legacy.resources) : []);
    const types = Array.isArray(legacy.types) ? legacy.types : (legacy.types && typeof legacy.types === "object" ? Object.values(legacy.types) : []);
    resources.forEach((item) => legacyResources.push(item));
    types.forEach((type) => legacyTypes.push(type));
  }
  return { resources: legacyResources, types: legacyTypes };
}

function getStudioResourceTypes() {
  const data = getStudioResourcesData();
  const raw = Array.isArray(data.types) ? data.types : (data.types && typeof data.types === "object" ? Object.values(data.types) : []);
  const custom = raw.map((value) => String(value || "").trim()).filter(Boolean);
  return [...new Set([...DEFAULT_RESOURCE_TYPES, ...custom])];
}

function getStudioResources() {
  const data = getStudioResourcesData();
  const raw = Array.isArray(data.resources) ? data.resources : (data.resources && typeof data.resources === "object" ? Object.values(data.resources) : []);
  const seen = new Set();
  return raw.map((item) => ({
    title: String(item?.title || "Resource").trim() || "Resource",
    type: String(item?.type || "Other").trim() || "Other",
    url: String(item?.url || "").trim(),
    notes: String(item?.notes || "").trim()
  })).filter((item) => {
    if (!(item.url || item.notes || item.title !== "Resource")) return false;
    const key = `${item.title.toLowerCase()}|||${item.url.toLowerCase()}|||${item.type.toLowerCase()}`;
...
function brandBiblePlainText(brand = currentBrand) {
  ...
  entries.forEach((entry) => {
    ...
    const heading = entry.category ? `${entry.category}: ` : "";
    grouped.get(entry.section).push(`${heading}${entry.text}`);
  });
  const links = getBrandBibleLinksForBrand(brand);
  const notesText = [...grouped.entries()].map(([section, notes]) => `${section}:\n${notes.map((note, index) => `${index + 1}. ${note}`).join("\n")}`).join("\n\n");
  const linksText = links.length ? `\n\nBrand Brief Links:\n${links.map((link, index) => `${index + 1}. ${link.label}: ${link.url}`).join("\n")}` : "";
  return notesText + linksText;
}
...
function assetReplyArray(request) {
  const raw = request?.replies;
  if (Array.isArray(raw)) return raw.filter(Boolean);
  if (raw && typeof raw === "object") return Object.values(raw).filter(Boolean).sort((a, b) => Number(a?.at || 0) - Number(b?.at || 0));
  return [];
}

function renderAssetReplyThread(request) {
  if (!els.assetReplyThread || !els.assetReplySection) return;
  const replies = assetReplyArray(request);
  els.assetReplySection.classList.toggle("hidden", !request);
  if (!request) {
    els.assetReplyThread.innerHTML = "";
    return;
  }
  const firstAt = request.createdAt || request.updatedAt;
  const opening = `<article class="asset-thread-item"><div class="asset-thread-meta"><strong>${escapeHtml(request.fromRole || "Designer")}</strong><span>to ${escapeHtml(request.toRole || "Head")}</span><time>${firstAt ? escapeHtml(new Date(Number(firstAt)).toLocaleString()) : ""}</time></div><p>${escapeHtml(request.message || "")}</p></article>`;
  const replyHtml = replies.map((reply) => `<article class="asset-thread-item"><div class="asset-thread-meta"><strong>${escapeHtml(reply.role || "Team")}</strong><time>${reply.at ? escapeHtml(new Date(Number(reply.at)).toLocaleString()) : ""}</time></div><p>${escapeHtml(reply.message || "")}</p></article>`).join("");
  els.assetReplyThread.innerHTML = opening + replyHtml;
}
...
function renderAssetRequest() {
  ...
  els.assetRequestNotice.innerHTML = `<div><strong>${needed ? "New assets requested" : "No new assets required"}</strong><p><b>${escapeHtml(request.fromRole || "Designer")}</b> → <b>${escapeHtml(request.toRole || "Head")}</b> · ${escapeHtml(request.message || "")}</p><small>${replies.length} repl${replies.length === 1 ? "y" : "ies"}${request.updatedAt ? ` · Updated ${escapeHtml(new Date(Number(request.updatedAt)).toLocaleString())}` : ""}</small></div><button type="button" class="ghost-btn asset-edit-btn">Open conversation</button>`;
...
function openAssetRequestDialog() {
  ...
  els.assetRequestFromRole.value = request?.fromRole || "Designer";
  els.assetRequestToRole.value = request?.toRole || "Head";
  if (request) {
    ...
    els.assetReplyRole.value = request.toRole || "Head";
  }
  renderAssetReplyThread(request);
...
async function saveAssetRequest(event) {
  ...
  if (!message) return alert("Please add a request message.");
  const existing = getAssetRequest();
  const payload = {
    needed: selected.value === "yes",
    fromRole: els.assetRequestFromRole.value,
    toRole: els.assetRequestToRole.value,
    message,
  ...
  const ok = await write(`assetRequests/${brandKey(currentBrand)}/${currentMonth}`, payload, "Asset request saved");
  if (ok) closeAssetRequestDialog();
}

async function sendAssetReply() {
  const request = getAssetRequest();
  if (!request) return alert("Save the asset request first, then add replies.");
  const message = els.assetReplyMessage.value.trim();
  if (!message) return alert("Please type a reply.");
  const replyId = crypto.randomUUID();
  const reply = { role: els.assetReplyRole.value, message, at: Date.now(), by: auth?.currentUser?.uid || "" };
  setSyncStatus("saving", "Sending reply…");
  try {
    await update(ref(db, `${ROOT_PATH}/assetRequests/${brandKey(currentBrand)}/${currentMonth}`), {
      [`replies/${replyId}`]: reply,
      updatedAt: Date.now(),
      updatedBy: auth.currentUser.uid
    });
    els.assetReplyMessage.value = "";
    const currentReplies = request.replies && typeof request.replies === "object" && !Array.isArray(request.replies) ? request.replies : {};
    renderAssetReplyThread({ ...request, replies: { ...currentReplies, [replyId]: reply }, updatedAt: Date.now() });
    setSyncStatus("saved", "Reply sent");
  } catch (error) {
    console.error(error);
    setSyncStatus("error", readableFirebaseError(error));
  }
...
function brandBibleHtmlBlock(brand = currentBrand) {
  ...
  const notesHtml = [...grouped.entries()].map(([section, notes]) => `<div class="bible-export-section"><b>${escapeHtml(section)}</b>${notes.map((note) => `${note.category ? `<strong class="bible-export-category">${escapeHtml(note.category)}</strong>` : ""}<ul>${(note.bullets || []).map((bullet) => `<li>${escapeHtml(bullet)}</li>`).join("")}</ul>`).join("")}</div>`).join("");
  const linksHtml = links.length ? `<div class="bible-export-section"><b>Brand Brief Links</b><ul>${links.map((link) => `<li><a href="${escapeHtml(link.url)}">${escapeHtml(link.label)}</a> — ${escapeHtml(link.url)}</li>`).join("")}</ul></div>` : "";
  return `<div class="brand-notes-box"><strong>Brand Bible / Notes</strong>${notesHtml}${linksHtml}</div>`;
}
...
async function exportPdf() {
  ...
  try {
    ...
    const drawSectionBox = (title, lines, fill = [247, 244, 238]) => {
      ...
      const allLines = wrapped.length ? wrapped : ["-"];
      ...
      while (offset < allLines.length) {
        // Keep every box above the footer. If there is not enough room for
        // a title plus at least one line, continue on a fresh page.
        let availableHeight = pageHeight - bottomMargin - y;
        if (availableHeight < titleHeight + lineHeight + 5) {
          addPageFooter();
          doc.addPage();
          y = 16;
        ...
        const maxLines = Math.max(1, Math.floor((availableHeight - titleHeight - 4) / lineHeight));
        const chunk = allLines.slice(offset, offset + maxLines);
        const boxHeight = titleHeight + chunk.length * lineHeight + 2;
        ...
        doc.setFontSize(9);
        doc.setTextColor(50);
        doc.text(pdfSafeText(continuation ? `${title} (continued)` : title), margin + 5, y + 6);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(75);
        doc.text(chunk, margin + 5, y + 11);
    ...
    if (bibleEntries.length) {
      ...
      bibleEntries.forEach((entry) => {
        ...
        if (entry.category) bibleLines.push(`  ${entry.category}`);
        (entry.bullets || []).forEach((bullet) => bibleLines.push(`    - ${bullet}`));
      });
      drawSectionBox("Brand Bible / Notes", bibleLines, [247, 244, 238]);
    }

    const bibleLinks = getBrandBibleLinksForBrand();
    if (bibleLinks.length) {
      ensureSpace(12 + bibleLinks.length * 7);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.setTextColor(50);
      doc.text("Brand Brief Links", margin, y);
      y += 6;
      bibleLinks.forEach((link, index) => {
        ensureSpace(7);
        const label = pdfSafeText(`${index + 1}. ${link.label}`);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(8.8);
        doc.setTextColor(30, 85, 180);
        doc.textWithLink(label, margin + 2, y, { url: link.url });
        const urlLines = doc.splitTextToSize(pdfSafeText(link.url), contentWidth - 55);
        doc.setTextColor(95);
        doc.text(urlLines, margin + 52, y);
        y += Math.max(6, urlLines.length * 4.1 + 1);
      });
      y += 3;
    }
    ...
    items.forEach((item, index) => {
      ...
      // Internal QC details are intentionally excluded from the client PDF.
      // They remain available in the Studio 5 Quality Control workspace.
      y += 7;
    });
...
const DEFAULT_BIBLE_SECTIONS = ["Things to Check","Monthly Content Focus"];
...
function getBrandBible(brand=currentBrand){ return remote.brandBibles?.[brandKey(brand)] || {}; }
function normalizeBibleSection(section=''){
  const clean=String(section||'').trim();
  return clean.toLowerCase()==='monthly content focus' ? 'Monthly Content Focus' : 'Things to Check';
}
function cleanBibleBullet(value=''){ return String(value||'').trim().replace(/^[•\-*–—]+\s*/, '').trim(); }
function isBibleHeading(line=''){
  const clean=String(line||'').trim().replace(/:$/, '');
  return clean && clean.length <= 60 && !/^[•\-*–—]/.test(String(line||'').trim()) && clean === clean.toUpperCase() && /[A-Z]/.test(clean);
}
function legacyTextToCategories(text='', fallback='General'){
  const lines=String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  if(!lines.length) return [];
  const groups=[]; let current=null;
  lines.forEach(line=>{
    if(isBibleHeading(line)){
      current={category:line.replace(/:$/, '').trim(),bullets:[]}; groups.push(current); return;
    }
    if(!current){ current={category:fallback,bullets:[]}; groups.push(current); }
    const bullet=cleanBibleBullet(line); if(bullet) current.bullets.push(bullet);
  });
  return groups.filter(g=>g.category && g.bullets.length);
}
function bibleEntries(brand=currentBrand){
  const b=getBrandBible(brand); let raw=[];
  if(Array.isArray(b.entries)) raw=b.entries;
  else if(b.entries&&typeof b.entries==='object') raw=Object.values(b.entries);
  else if(brand===currentBrand) { const legacy=getBrandResources()?.notes; if(legacy) raw=[{section:'Things to Check',text:legacy}]; }
  const out=[];
  raw.filter(Boolean).forEach(x=>{
    const section=normalizeBibleSection(x.section);
    if(Array.isArray(x.bullets)){
      const bullets=x.bullets.map(cleanBibleBullet).filter(Boolean);
      if(bullets.length) out.push({section,category:String(x.category||'').trim(),bullets,text:bullets.join('\n'),mode:x.mode==='bullets'?'bullets':'category'});
      return;
    }
    legacyTextToCategories(x.text||'', x.category||'General').forEach(g=>out.push({section,category:g.category,bullets:g.bullets,text:g.bullets.join('\n'),mode:'category'}));
  });
  return out;
}
function getBibleCategories(){ return [...DEFAULT_BIBLE_SECTIONS]; }
function renderBrandBibleReminder(){
  const el=document.querySelector('#brandBibleReminder'); if(!el)return;
  const entries=bibleEntries(); el.classList.toggle('hidden',!entries.length);
  const bullets=entries.reduce((n,x)=>n+x.bullets.length,0);
  el.innerHTML=entries.length?`<strong>${escapeHtml(currentBrand)} Brand Bible</strong><span>${entries.length} categor${entries.length===1?'y':'ies'} · ${bullets} bullet${bullets===1?'':'s'} across Things to Check and Monthly Content Focus — automatically shown in Add Content.</span>`:'';
}
function openBrandBible(){
  document.querySelector('#brandBibleTitle').textContent=`${currentBrand} — Brand Bible`;
  renderBrandBibleWorkspace(); renderBrandBibleLinks(); document.querySelector('#brandBibleDialog').showModal();
}
function renderBrandBibleWorkspace(){
  const list=document.querySelector('#brandBibleList'); if(!list)return; list.innerHTML='';
  const entries=bibleEntries();
  DEFAULT_BIBLE_SECTIONS.forEach(section=>{
    const group=document.createElement('section'); group.className='bible-category-group'; group.dataset.category=section;
    group.innerHTML=`<div class="bible-category-head"><div><span class="eyebrow">Brand Bible</span><h4>${escapeHtml(section)}</h4><p>${section==='Things to Check'?'Permanent brand rules, grouped by category.':'Month-specific priorities, grouped by category.'}</p></div><button type="button" class="ghost-btn bible-add-category">+ Add Category</button></div><div class="bible-category-notes"></div>`;
    const wrap=group.querySelector('.bible-category-notes'); const sectionEntries=entries.filter(x=>x.section===section);
    if(sectionEntries.length) sectionEntries.forEach(x=>addBibleCategoryRow(section,x.category,x.bullets,wrap,x.mode));
    else wrap.innerHTML=`<div class="bible-empty-note">No categories saved yet. Select “+ Add Category” to begin.</div>`;
    group.querySelector('.bible-add-category').addEventListener('click',()=>{ wrap.querySelector('.bible-empty-note')?.remove(); const row=addBibleCategoryRow(section,'',[],wrap); row.querySelector('.bible-category-input')?.focus(); });
    list.appendChild(group);
  });
}
function addBibleCategoryRow(section,category='',bullets=[],target=null,mode='category'){
  const row=document.createElement('article'); row.className='bible-entry bible-structured-entry'; row.dataset.section=normalizeBibleSection(section);
  const bulletText=(Array.isArray(bullets)?bullets:[]).map(cleanBibleBullet).filter(Boolean).map(x=>`• ${x}`).join('\n');
  const normalizedMode=mode==='bullets'?'bullets':'category';
  row.innerHTML=`<div class="bible-entry-main"><label class="bible-mode-field"><span>Layout</span><select class="bible-mode-select"><option value="category" ${normalizedMode==='category'?'selected':''}>Category + Bullet List</option><option value="bullets" ${normalizedMode==='bullets'?'selected':''}>Bullet List Only</option></select></label><label class="bible-category-field"><span>Category / Heading</span><input class="bible-category-input" type="text" placeholder="e.g. POSITIONING, BRAND FEEL, VISUALS" value="${escapeHtml(category)}"></label><label class="bible-bullets-field"><span>Bullet List</span><textarea class="bible-bullets-input" rows="6" placeholder="Add one bullet per line. You can type with or without • or -. ">${escapeHtml(bulletText)}</textarea></label></div><button type="button" class="danger-btn bible-delete">Delete</button>`;
  const syncMode=()=>{ const bulletsOnly=row.querySelector('.bible-mode-select').value==='bullets'; row.classList.toggle('bible-bullets-only',bulletsOnly); const field=row.querySelector('.bible-category-field'); const input=row.querySelector('.bible-category-input'); field.classList.toggle('hidden',bulletsOnly); input.required=!bulletsOnly; if(bulletsOnly) input.value=''; };
  row.querySelector('.bible-mode-select').addEventListener('change',syncMode); syncMode();
  row.querySelector('.bible-delete').addEventListener('click',()=>{ const wrap=row.parentElement; row.remove(); if(wrap&&!wrap.querySelector('.bible-entry')) wrap.innerHTML='<div class="bible-empty-note">No items saved yet. Select “+ Add Category” to begin.</div>'; });
  (target||document.querySelector('#brandBibleList')).appendChild(row); return row;
}

function addBibleNoteRow(section,text='',target=null){ const parsed=legacyTextToCategories(text); const first=parsed[0]||{category:'',bullets:text?[cleanBibleBullet(text)]:[]}; return addBibleCategoryRow(section,first.category,first.bullets,target); }
function addBibleEntryRow(section='Things to Check',text=''){ const normalized=normalizeBibleSection(section); const group=[...document.querySelectorAll('.bible-category-group')].find(x=>x.dataset.category===normalized); const wrap=group?.querySelector('.bible-category-notes'); if(wrap){wrap.querySelector('.bible-empty-note')?.remove();return addBibleNoteRow(normalized,text,wrap);}return null; }
function renderBrandBibleLinks(){
  const list=document.querySelector('#brandBibleLinksList'); if(!list)return; list.innerHTML='';
  const links=getBrandBibleLinksForBrand();
  if(!links.length){ list.innerHTML='<div class="bible-empty-note">No Brand Brief links saved yet. Select “+ Add Link” to add one.</div>'; return; }
  links.forEach(link=>addBrandBibleLinkRow(link.label,link.url,list));
}
function addBrandBibleLinkRow(label='',url='',target=null){
  const list=target||document.querySelector('#brandBibleLinksList'); if(!list)return null; list.querySelector('.bible-empty-note')?.remove();
  const row=document.createElement('div'); row.className='brand-bible-link-row';
  row.innerHTML=`<label><span>Link Label</span><input class="brand-bible-link-label" type="text" placeholder="e.g. Main Brand Brief" value="${escapeHtml(label)}"></label><label><span>URL</span><input class="brand-bible-link-url" type="url" placeholder="https://..." value="${escapeHtml(url)}"></label><button type="button" class="danger-btn">Delete</button>`;
  row.querySelector('button').addEventListener('click',()=>{ row.remove(); if(!list.querySelector('.brand-bible-link-row')) list.innerHTML='<div class="bible-empty-note">No Brand Brief links saved yet. Select “+ Add Link” to add one.</div>'; });
  list.appendChild(row); return row;
}
async function saveBrandBible(e){
  e.preventDefault(); const rows=[...document.querySelectorAll('#brandBibleDialog .bible-entry')]; const entries=[];
  for(const r of rows){
    const section=normalizeBibleSection(r.dataset.section), mode=r.querySelector('.bible-mode-select')?.value==='bullets'?'bullets':'category', category=mode==='bullets'?'':(r.querySelector('.bible-category-input')?.value.trim()||''), bullets=String(r.querySelector('.bible-bullets-input')?.value||'').split(/\r?\n/).map(cleanBibleBullet).filter(Boolean);
    if(!category&&!bullets.length)continue;
    if(mode==='category'&&!category)return alert('Please add a category / heading, or switch the item to Bullet List Only.');
    if(!bullets.length)return alert(`Please add at least one bullet${category?` under ${category}`:''}.`);
    entries.push({section,mode,category,bullets});
  }
  const links=[];
  for(const row of document.querySelectorAll('#brandBibleLinksList .brand-bible-link-row')){
    const label=row.querySelector('.brand-bible-link-label')?.value.trim()||'Brand Brief';
    const url=row.querySelector('.brand-bible-link-url')?.value.trim()||'';
    if(!url&&!label)continue;
    if(!url)return alert('Please add a URL for each Brand Brief link, or delete the empty link row.');
    links.push({label:label||'Brand Brief',url});
  }
  const ok=await write(`brandBibles/${brandKey(currentBrand)}`,{version:3,categories:[...DEFAULT_BIBLE_SECTIONS],entries,links,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Bible saved'); if(ok)document.querySelector('#brandBibleDialog').close();
}


function getQcLeadReminders(brand = currentBrand) {
  const data = remote.qcLeadReminders?.[brandKey(brand)] || {};
  return Object.entries(data)
    .map(([id, item]) => ({ id, ...(item && typeof item === "object" ? item : {}) }))
    .sort((a, b) => Number(a.order ?? a.createdAt ?? 0) - Number(b.order ?? b.createdAt ?? 0));
}

function renderQcLeadReminders() {
  const list = document.querySelector("#qcRemindersList");
  const brandName = document.querySelector("#qcRemindersBrandName");
  if (!list) return;
  if (brandName) brandName.textContent = currentBrand;
  const items = getQcLeadReminders();
  list.innerHTML = "";
  if (!items.length) {
    list.innerHTML = `<div class="qc-reminders-empty"><strong>No QC lead reminders for ${escapeHtml(currentBrand)} yet.</strong><p>Add the recurring things the QC lead should remember to check for this brand.</p></div>`;
    return;
  }
  items.forEach((item) => addQcLeadReminderCard(item, list));
}

function addQcLeadReminderCard(item = {}, target = null) {
  const list = target || document.querySelector("#qcRemindersList");
  if (!list) return null;
  list.querySelector(".qc-reminders-empty")?.remove();
  const card = document.createElement("article");
  card.className = "qc-reminder-card";
  card.dataset.id = item.id || "";
  card.innerHTML = `
    <div class="qc-reminder-card-head">
      <div>
        <span class="eyebrow">QC reminder</span>
        <small class="qc-reminder-save-state">${item.id ? "Saved" : "New reminder"}</small>
      </div>
      <button type="button" class="danger-btn qc-reminder-delete">Delete</button>
    </div>
    <label class="qc-reminder-title-field">Title / Check Area
      <input class="qc-reminder-title" type="text" placeholder="e.g. Product imagery, Copy, Location details" value="${escapeHtml(item.title || "")}">
    </label>
    <label>Things to Check / Reminder
      <textarea class="qc-reminder-body" rows="7" placeholder="Add the exact things the QC lead should remember to verify for this brand...">${escapeHtml(item.body || "")}</textarea>
    </label>
    <div class="qc-reminder-footer">
      <small>${item.updatedAt ? `Last updated ${escapeHtml(new Date(Number(item.updatedAt)).toLocaleString())}` : "Not saved yet"}</small>
      <button type="button" class="primary-btn qc-reminder-save">${item.id ? "Save Changes" : "Save Reminder"}</button>
    </div>`;

  const markDirty = () => {
    const state = card.querySelector(".qc-reminder-save-state");
    if (state) state.textContent = card.dataset.id ? "Unsaved changes" : "New reminder";
  };
  card.querySelector(".qc-reminder-title")?.addEventListener("input", markDirty);
  card.querySelector(".qc-reminder-body")?.addEventListener("input", markDirty);
  card.querySelector(".qc-reminder-save")?.addEventListener("click", () => saveQcLeadReminderCard(card));
  card.querySelector(".qc-reminder-delete")?.addEventListener("click", () => deleteQcLeadReminderCard(card));
  list.appendChild(card);
  if (!item.id) card.querySelector(".qc-reminder-title")?.focus();
  return card;
}

async function saveQcLeadReminderCard(card) {
  const title = card.querySelector(".qc-reminder-title")?.value.trim() || "";
  const body = card.querySelector(".qc-reminder-body")?.value.trim() || "";
  if (!title && !body) return alert("Please add a title or reminder before saving.");
  const id = card.dataset.id || crypto.randomUUID();
  const existing = getQcLeadReminders().find((item) => item.id === id);
  const payload = {
    title,
    body,
    order: Number(existing?.order ?? existing?.createdAt ?? Date.now()),
    createdAt: Number(existing?.createdAt) || Date.now(),
    updatedAt: Date.now(),
    updatedBy: auth.currentUser.uid
  };
  const ok = await write(`qcLeadReminders/${brandKey(currentBrand)}/${id}`, payload, "QC lead reminder saved");
  if (ok) card.dataset.id = id;
}

async function deleteQcLeadReminderCard(card) {
  const id = card.dataset.id;
  const title = card.querySelector(".qc-reminder-title")?.value.trim() || "this reminder";
  if (!id) {
    card.remove();
    if (!document.querySelector("#qcRemindersList .qc-reminder-card")) renderQcLeadReminders();
    return;
  }
  if (!confirm(`Delete “${title}” from ${currentBrand}'s QC lead reminders?`)) return;
  setSyncStatus("saving", "Saving…");
  try {
    await remove(ref(db, `${ROOT_PATH}/qcLeadReminders/${brandKey(currentBrand)}/${id}`));
    await update(ref(db, `${ROOT_PATH}/meta`), { updatedAt: serverTimestamp(), updatedBy: auth.currentUser.uid });
    setSyncStatus("saved", "QC lead reminder deleted");
  } catch (error) {
    console.error(error);
    setSyncStatus("error", readableFirebaseError(error));
    alert(`Delete failed: ${readableFirebaseError(error)}`);
...
function renderOtherResourcesView(){
  const list=document.querySelector('#otherResourcesViewList');
  const summary=document.querySelector('#otherResourcesViewSummary');
  if(!list||!summary)return;
  const items=getStudioResources();
  const customRaw=getStudioResourcesData().types;
  const custom=(Array.isArray(customRaw)?customRaw:(customRaw&&typeof customRaw==='object'?Object.values(customRaw):[])).map(x=>String(x||'').trim()).filter(Boolean);
  summary.innerHTML=`<div class="resource-view-stat"><span>Library</span><strong>Studio 5</strong></div><div class="resource-view-stat"><span>Saved Resources</span><strong>${items.length}</strong></div><div class="resource-view-stat"><span>Custom Types</span><strong>${custom.length}</strong></div>`;
  if(!items.length){
    list.innerHTML='<div class="other-resources-view-empty"><strong>No other resources saved yet.</strong><p>Use “+ Add / Manage Resources” to add a content tracker, marketing planner, Google Sheet, PDF, folder, dashboard or another useful material for the Studio 5 team.</p></div>';
    return;
  }
  list.innerHTML='';
  items.forEach(item=>{
    const card=document.createElement('article');
    card.className='other-resource-view-card';
    const safeUrl=escapeHtml(item.url||'');
    card.innerHTML=`<div class="other-resource-view-card-top"><span class="category-pill">${escapeHtml(item.type||'Other')}</span><a class="other-resource-open-link" href="${safeUrl}" target="_blank" rel="noopener noreferrer">Open Resource ↗</a></div><h4>${escapeHtml(item.title||'Untitled Resource')}</h4><p>${escapeHtml(item.notes||'No notes added.')}</p><div class="other-resource-url-preview">${safeUrl}</div>`;
    list.appendChild(card);
  });
}

function closeOtherResources(){ document.querySelector('#otherResourcesDialog')?.close(); }
function openOtherResources(){
  document.querySelector('#otherResourcesTitle').textContent='Studio 5 — Other Resources';
  renderOtherResourceTypes();
  renderOtherResources();
  document.querySelector('#otherResourcesDialog')?.showModal();
}
function resourceTypeOptions(selected='Other'){
  const types=getStudioResourceTypes();
...
function renderOtherResources(){
  const list=document.querySelector('#otherResourcesList'); if(!list)return; list.innerHTML='';
  const items=getStudioResources();
  if(!items.length){list.innerHTML='<div class="resource-empty-state">No additional resources saved yet. Select “+ Add Resource” to add a tracker, planner, PDF, folder, or other useful link.</div>';return;}
  items.forEach(item=>addOtherResourceRow(item,list));
}
function addOtherResourceRow(item={},target=null){
  const list=target||document.querySelector('#otherResourcesList'); if(!list)return null;
  list.querySelector('.resource-empty-state')?.remove();
  const row=document.createElement('article'); row.className='other-resource-row';
  const title=String(item.title||''), type=String(item.type||'Google Sheet'), url=String(item.url||''), notes=String(item.notes||'');
  row.innerHTML=`<div class="other-resource-fields"><label class="resource-title-field"><span>Resource Name</span><input class="other-resource-title" type="text" placeholder="e.g. August Content Tracker" value="${escapeHtml(title)}"></label><label class="resource-type-field"><span>Type</span><select class="other-resource-type">${resourceTypeOptions(type)}</select></label><label class="resource-link-field"><span>Link</span><input class="other-resource-url" type="url" placeholder="https://..." value="${escapeHtml(url)}"></label><label class="resource-notes-field"><span>Notes</span><textarea class="other-resource-notes" rows="2" placeholder="What this is for, access notes, owner, update schedule, etc.">${escapeHtml(notes)}</textarea></label></div><button type="button" class="danger-btn other-resource-delete">Delete</button>`;
  row.querySelector('.other-resource-delete').addEventListener('click',()=>{row.remove();if(!list.querySelector('.other-resource-row'))list.innerHTML='<div class="resource-empty-state">No additional resources saved yet. Select “+ Add Resource” to add one.</div>';});
  list.appendChild(row); return row;
}
function renderOtherResourceTypes(){
  const list=document.querySelector('#resourceTypeList'); if(!list)return; list.innerHTML='';
  const stored=getStudioResourcesData().types;
  const custom=(Array.isArray(stored)?stored:(stored&&typeof stored==='object'?Object.values(stored):[])).map(x=>String(x||'').trim()).filter(Boolean);
  if(!custom.length) list.innerHTML='<div class="resource-type-help">Default types are always available. Add custom types below when your team needs something more specific.</div>';
  custom.forEach(value=>addResourceTypeRow(value,list));
}
function addResourceTypeRow(value='',target=null){
  const list=target||document.querySelector('#resourceTypeList'); if(!list)return null;
  list.querySelector('.resource-type-help')?.remove();
  const row=document.createElement('div'); row.className='resource-type-row';
  row.innerHTML=`<input class="resource-type-input" type="text" placeholder="e.g. Campaign Dashboard" value="${escapeHtml(value)}"><button type="button" class="danger-btn">Delete</button>`;
  row.querySelector('button').addEventListener('click',()=>{row.remove();refreshResourceTypeSelects();if(!list.querySelector('.resource-type-row'))list.innerHTML='<div class="resource-type-help">Default types are always available. Add custom types below when your team needs something more specific.</div>';});
  row.querySelector('input').addEventListener('input',refreshResourceTypeSelects);
  list.appendChild(row); refreshResourceTypeSelects(); return row;
}
function currentCustomResourceTypes(){
  return [...document.querySelectorAll('#resourceTypeList .resource-type-input')].map(el=>el.value.trim()).filter(Boolean);
}
function refreshResourceTypeSelects(){
  const types=[...new Set([...DEFAULT_RESOURCE_TYPES,...currentCustomResourceTypes()])];
  document.querySelectorAll('#otherResourcesList .other-resource-type').forEach(select=>{
    const current=select.value; const opts=[...types]; if(current&&!opts.includes(current))opts.push(current);
    select.innerHTML=opts.map(type=>`<option value="${escapeHtml(type)}" ${type===current?'selected':''}>${escapeHtml(type)}</option>`).join('');
  });
}
async function saveOtherResources(e){
  e.preventDefault();
  const types=[...new Set(currentCustomResourceTypes())];
  const resources=[];
  for(const row of document.querySelectorAll('#otherResourcesList .other-resource-row')){
    const title=row.querySelector('.other-resource-title')?.value.trim()||'';
    const type=row.querySelector('.other-resource-type')?.value.trim()||'Other';
    const url=row.querySelector('.other-resource-url')?.value.trim()||'';
    const notes=row.querySelector('.other-resource-notes')?.value.trim()||'';
    if(!title&&!url&&!notes) continue;
    if(!title) return alert('Please add a Resource Name for each saved resource.');
    if(!url) return alert(`Please add a link for “${title}”, or delete that resource row.`);
    resources.push({title,type,url,notes});
  }
  const ok=await write('studioResources',{types,resources,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Studio 5 resources saved');
  if(ok){ closeOtherResources(); renderOtherResourcesView(); }
}

function bibleTextLines(text){ return String(text||'').split(/\r?\n/).map(cleanBibleBullet).filter(Boolean); }
function brandBibleSectionHtml(section,entries){
  const groups=entries.filter(x=>x.section===section); if(!groups.length)return '';
  return `<section class="content-rule-section"><strong>${escapeHtml(section)}</strong>${groups.map(g=>`<div class="content-rule-category">${g.category?`<b>${escapeHtml(g.category)}</b>`:''}<ul>${g.bullets.map(line=>`<li>${escapeHtml(line)}</li>`).join('')}</ul></div>`).join('')}</section>`;
}
function renderContentBrandRules(){
  const el=document.querySelector('#contentBrandRules'); if(!el)return; const entries=bibleEntries();
  if(!entries.length){el.innerHTML=`<div class="content-rules-head"><strong>${escapeHtml(currentBrand)} — no Brand Bible notes saved yet</strong><span>Ask for clarification rather than assume.</span></div>`;return;}
  el.innerHTML=`<div class="content-rules-head"><strong>${escapeHtml(currentBrand)} — Brand Bible</strong><span>Automatic Brand Bible reminder</span></div><div class="content-rules-sections">${DEFAULT_BIBLE_SECTIONS.map(s=>brandBibleSectionHtml(s,entries)).join('')}</div>`;
}

let contentCreationMode = "ai";

function setContentCreationMode(mode = "manual") {
  contentCreationMode = mode === "ai" ? "ai" : "manual";
  document.querySelectorAll("[data-content-mode]").forEach((button) => {
    button.classList.toggle("active", button.dataset.contentMode === contentCreationMode);
  });
  document.querySelector("#aiAssistantPanel")?.classList.toggle("hidden", contentCreationMode !== "ai");
}

function setAiGenerationStatus(message = "", state = "") {
  const el = document.querySelector("#aiGenerationStatus");
  if (!el) return;
...
function getAiBrandSources(brand = currentBrand) {
  const bibleEntriesData = getBrandBibleEntriesForBrand(brand);
  const bibleLinks = getBrandBibleLinksForBrand(brand);
  const sopObject = remote.sopResources?.[brandKey(brand)] || {};
  const sopResources = Object.entries(sopObject).map(([id, item]) => ({
    id,
    type: item?.type || "link",
    title: item?.title || "Brand Brief",
    notes: item?.notes || "",
    url: item?.url || "",
    dataUrl: item?.dataUrl || "",
    fileName: item?.fileName || ""
  }));
  return { bibleEntries: bibleEntriesData, bibleLinks, sopResources };
}

function refreshAiSourceSummary() {
  const { bibleEntries, bibleLinks, sopResources } = getAiBrandSources();
  const count = bibleEntries.length + bibleLinks.length + sopResources.length;
  const title = document.querySelector("#aiSourceTitle");
  const countEl = document.querySelector("#aiSourceCount");
  const chips = document.querySelector("#aiSourceChips");
  if (title) title.textContent = `${currentBrand} sources`;
  if (countEl) countEl.textContent = count
    ? `${bibleEntries.length} Brand Bible item${bibleEntries.length === 1 ? "" : "s"}, ${bibleLinks.length + sopResources.length} Brand Brief / SOP source${bibleLinks.length + sopResources.length === 1 ? "" : "s"}`
    : "No saved Brand Bible or Brand Brief sources yet. AI can still use your idea, but brand-specific accuracy will be limited.";
  if (chips) {
    const chipHtml = [];
    if (bibleEntries.length) chipHtml.push(`<span>✓ Brand Bible</span>`);
    if (bibleLinks.length) chipHtml.push(`<span>✓ ${bibleLinks.length} Bible link${bibleLinks.length === 1 ? "" : "s"}</span>`);
    if (sopResources.some((item) => item.type === "link")) chipHtml.push(`<span>✓ Brand Brief link</span>`);
    if (sopResources.some((item) => item.type === "pdf")) chipHtml.push(`<span>✓ Brand Brief PDF</span>`);
    chips.innerHTML = chipHtml.join("") || `<span class="muted-chip">No brand sources saved</span>`;
  }
}

function collectCurrentContentFields() {
  const ids = {
    title: "contentTitle", type: "contentType", platform: "contentPlatform", pillar: "contentPillar",
    contentGoal: "contentGoal", productFocus: "productFocus", hook: "contentHook", audienceBenefit: "audienceBenefit",
    contentFocus: "contentFocus", caption: "contentCaption", cta: "contentCta", requiredAssets: "requiredAssets",
    referenceContent: "referenceContent", avoidContent: "avoidContent", visual: "contentVisual"
  };
  return Object.fromEntries(Object.entries(ids).map(([key, id]) => [key, document.querySelector(`#${id}`)?.value?.trim() || ""]));
}

function applyAiContentFields(generated, mode = "all") {
  const mapping = {
    title: "contentTitle", type: "contentType", platform: "contentPlatform", pillar: "contentPillar",
    contentGoal: "contentGoal", productFocus: "productFocus", hook: "contentHook", audienceBenefit: "audienceBenefit",
    contentFocus: "contentFocus", caption: "contentCaption", cta: "contentCta", requiredAssets: "requiredAssets",
  ...
  for (const [key, id] of Object.entries(mapping)) {
    ...
    if (mode === "missing" && String(el.value || "").trim()) continue;
    if (el.tagName === "SELECT") {
      const match = [...el.options].find((option) => option.value.toLowerCase() === String(value).trim().toLowerCase());
...
async function generateContentWithAi() {
  if (!AI_BACKEND_URL || AI_BACKEND_URL.includes("YOUR-VERCEL")) {
    return alert("AI Auto-Fill needs the external backend URL first. Open ai-config.js and paste your Vercel deployment URL.");
  }
  if (!auth?.currentUser) return alert("The Studio 5 workspace is still signing in. Please wait a moment and try again.");
  const idea = document.querySelector("#aiContentIdea")?.value.trim() || "";
  if (!idea) return alert("Please add an idea or topic first.");
  const button = document.querySelector("#generateContentAiBtn");
  const another = document.querySelector("#generateAnotherAiBtn");
  const generationMode = document.querySelector("#aiGenerationMode")?.value || "all";
  const preferredType = document.querySelector("#aiPreferredType")?.value || "";
  const existingFields = collectCurrentContentFields();
  button.disabled = true;
  another.disabled = true;
  setAiGenerationStatus(`Reviewing ${currentBrand} Brand Bible and Brand Brief / SOP sources…`, "loading");
  try {
    ...
    let response;
    try {
      response = await fetch(AI_BACKEND_URL, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) throw new Error(payload?.error || `AI backend error (${response.status})`);
    if (!payload.fields) throw new Error("The AI returned an incomplete brief.");
    applyAiContentFields(payload.fields, generationMode);
    const sourceParts = [];
    if (payload.sourcesUsed?.length) sourceParts.push(`${payload.sourcesUsed.length} source${payload.sourcesUsed.length === 1 ? "" : "s"} used`);
    if (payload.sourcesSkipped?.length) sourceParts.push(`${payload.sourcesSkipped.length} source${payload.sourcesSkipped.length === 1 ? "" : "s"} could not be read`);
    setAiGenerationStatus(`Generated successfully${sourceParts.length ? ` · ${sourceParts.join(" · ")}` : ""}. Review and edit every field before saving.`, "success");
  } catch (error) {
    console.error("AI content generation failed:", error);
    const message = error?.message?.replace(/^FirebaseError:\s*/i, "") || "AI generation failed.";
    setAiGenerationStatus(`${message} You can continue with Manual Fill.`, "error");
  } finally {
...
function renderSopResources(){ const list=document.querySelector('#sopResourceList'); if(!list)return; const items=getSopResources(); list.innerHTML=items.length?'':`<div class="content-links-empty">No Brand Brief resources saved for ${escapeHtml(currentBrand)} yet.</div>`; items.forEach(x=>{const card=document.createElement('article');card.className='sop-resource-card'; const link=x.type==='pdf'&&x.dataUrl?`<a href="${escapeHtml(x.dataUrl)}" download="${escapeHtml(x.fileName||x.title)}">Open / download Brand Brief PDF</a>`:x.url?`<a href="${escapeHtml(x.url)}" target="_blank" rel="noopener">Open Brand Brief link</a>`:''; card.innerHTML=`<div><span class="category-pill">${x.type==='pdf'?'PDF':'LINK'}</span><h4>${escapeHtml(x.title||'Untitled Brand Brief')}</h4><p>${escapeHtml(x.notes||'No notes.')}</p>${link}</div><button class="danger-btn">Delete</button>`;card.querySelector('button').onclick=()=>deleteSopResource(x.id);list.appendChild(card);}); }
...
async function saveSopResource(e){e.preventDefault();const type=document.querySelector('#sopResourceType').value,title=document.querySelector('#sopResourceTitle').value.trim(),notes=document.querySelector('#sopResourceNotes').value.trim(),url=document.querySelector('#sopResourceUrl').value.trim(),file=document.querySelector('#sopResourceFile').files?.[0];if(type==='link'&&!url)return alert('Please add the Brand Brief link.');if(type==='pdf'&&!file)return alert('Please choose a Brand Brief PDF.');if(file&&file.size>4*1024*1024)return alert('Please upload a PDF no larger than 4 MB.');let dataUrl='',fileName='';if(file){dataUrl=await readFileAsDataUrl(file);fileName=file.name;}const id=crypto.randomUUID();const ok=await write(`sopResources/${brandKey(currentBrand)}/${id}`,{type,title,notes,url:type==='link'?url:'',dataUrl,fileName,size:file?.size||0,updatedAt:Date.now(),updatedBy:auth.currentUser.uid},'Brand Brief saved');if(ok)closeSopResourceDialog();}
async function deleteSopResource(id){if(!confirm('Delete this Brand Brief resource?'))return;await remove(ref(db,`${ROOT_PATH}/sopResources/${brandKey(currentBrand)}/${id}`));}
function addQcRow() {}
  ...
  items.forEach((item) => {
    ...
    card.innerHTML = `
      ...
      <aside class="qc-card-side">
        ...
        <button type="button" class="primary-btn qc-quick-review" ${getQcOverallState(item) === "approved" ? "disabled" : ""}>${getQcOverallState(item) === "approved" ? "QC Complete" : "Quick Review"}</button>
      </aside>`;
    ...
    card.querySelector(".qc-quick-review")?.addEventListener("click", () => {
      const state = getQcOverallState(item);
      const stage = state === "internal" ? "internal" : state === "nicole" ? "nicole" : "creator";
      openQcStage(item.id, stage);
    });
...
function bindQcWorkspaceEvents() {
  ...
  document.querySelectorAll(".stage-qc-dialog").forEach((dialog) => {
    ...
    dialog.querySelector(".stage-check-all")?.addEventListener("click", () => {
      dialog.querySelectorAll(".qc-check").forEach((check) => { check.checked = true; });
      const msg = dialog.querySelector(".stage-qc-message");
      if (msg) msg.textContent = "All checks marked complete. Review the notes, then approve the stage when ready.";
    });
